...
async def dialogue_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите название произведения", reply_markup=ReplyKeyboardRemove()
    )
...
async def get_chapter(message: Message, state: FSMContext):
    ...
    document = BufferedInputFile(
        data_for_db.text.encode("utf-8"), filename=f"Глава_{data_for_db.chapter}.md"
    )
    await message.answer_document(document, caption=f"Вот глава {data_for_db.chapter}")
    await message.answer(
        "Продолжим со следующей главы?", reply_markup=dialogue_continue
    )
...
async def stop_query(callback: CallbackQuery):
    await callback.message.answer(
        "Приятно было работать с тобой", reply_markup=kb.start
    )
